...
Public Class AdminManualSpeedsettings
#Region "声明"
    '实例化数据库连接
...
#Region "初始化"
    ...
    Private Sub AdminManualSpeedsettings_Load(sender As Object, e As EventArgs) Handles MyBase.Load

        Try
            '注册窗体的键盘事件
            Me.KeyPreview = True
            '从数据库中提取生成UI的设置（包括个数，地址，类型）
            UIarrayList = conn.GetRowsarrayList("Speed.mdb", "手动速度")
            '创建UI更新线程
            ...
            If (UBound(UIarrayList) > 0) Then
                '注意：生成的控件间隔距离为35,窗体顶部有大约100的高度为空白，所以窗体的高度为 个数*35+100
                '宽度不变
...
#End Region
    ...
    Private Sub UI_Keydown(sender As Object, e As KeyEventArgs)
        ...
        '创建UI并添加数据到UI时同样会触发该事件，增加一个判断禁止,将数据
        '写入设备
...
#Region "保存数据"
    ...
    Private Sub Savedata()
        ...
        con.ChangedData(AppPath, "手动速度", tempdata)
        MsgBox("已保存数据")
    End Sub
